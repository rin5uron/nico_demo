...
[2025年1008: Vercelでのブランチ別デプロイ](#20251008-vercelでのブランチ別デプロイ)
[2025年1008: git worktreeについて](#20251008-git-worktreeについて)
...
# 2025年10月8日: Vercelでのブランチ別デプロイ

### Vercelの自動デプロイ機能
VercelはGitリポジトリと連携することで、ブランチごとに異なるURLを自動で生成する機能を持つ。

- **本番環境 (Production):**
  - `main`や`master`ブランチが対象。
  - `プロジェクト名.vercel.app` のような固定のURLにデプロイされる。

- **プレビュー環境 (Preview):**
  - `main`以外のブランチ（例: `pattern-b`）がプッシュされると、自動でデプロイが始まる。
  - `プロジェクト名-git-ブランチ名-アカウント名.vercel.app` のような、ブランチごとにユニークなURLが生成される。

### ワークフロー
1.  **VercelとGitリポジトリを連携:**
    - Vercelのダッシュボードで対象リポジトリをインポートする（初回のみ）。
2.  **新しいブランチをプッシュ:**
    - ローカルで作成した新しいブランチ（例: `pattern-b`）を `git push origin pattern-b` のようにリモートへプッシュする。
3.  **URLの自動生成:**
    - Vercelがプッシュを検知し、自動でプレビュー環境を構築。本番URLとは別の、共有可能なプレビューURLが発行される。

### 利点
- `worktree`で作成した別パターンのデザインなどを、クライアントに実際のURLで簡単に共有・確認してもらえる。
- 本番環境に影響を与えることなく、安全に新しいバージョンを試すことができる。

---

# 2025年10月8日: git worktreeについて

### git worktreeとは？
- 1つのリポジトリから、複数のブランチをそれぞれ別のディレクトリにチェックアウトして、同時に作業するためのGitコマンド。

### `git checkout`との違い
- `checkout`: 1つの作業ディレクトリ内で、ブランチを切り替えて使う。一度に1つのブランチしか作業できない。
- `worktree`: 複数の作業ディレクトリを用意する。ブランチごとにディレクトリが分かれるため、同時に複数のブランチで作業できる。

### 主な利点
- `git stash`を使わずに、急な修正や別パターンの試作など、並行作業が安全かつ簡単に行える。
- ブランチごとに依存関係が違う場合（例: `npm install`の内容が違う）でも、ディレクトリが分かれているため問題なく対応できる。

### 今回の使用例
1. `git branch pattern-b` で新バージョン用のブランチを作成。
2. `git worktree add ../nico_demo_pattern_b pattern-b` で、`pattern-b`ブランチを`nico_demo_pattern_b`ディレクトリに展開。
3. これにより、`nico_demo`（完成版）と`nico_demo_pattern_b`（新パターン）を同時に起動し、別々のURLで確認できた。

### よくある質問 (Q&A)

**Q: worktreeにすると別リポジトリでもマージできる？**
A: いいえ、worktreeは**単一のリポジトリ内**で活動します。見た目は別のフォルダでも、内部では同じ`.git`データベースを共有しています。別のリポジポジトリと連携するには`git remote`などを使います。

**Q: コミットはローカル保存？**
A: はい。`git commit`はどのworktreeで行っても、まずローカルのリポジトリに保存されます。これは通常のgit操作と同じです。

**Q: pushしたらどうなる？**
A: worktreeで作業しているブランチ（例: `pattern-b`）を`git push`すると、そのブランチがリモートリポジトリ（GitHubなど）にプッシュされます。これにより、リモートの`main`ブランチに影響を与えずに、新しいバージョンのブランチを共有できます。
